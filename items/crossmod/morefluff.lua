if not (FLUFF and FLUFF.rewrite) then
	sendInfoMessage("This crossmod content requires the rewrite version of More Fluff, aborting...", "TOGAPack - MoreFluff")
	return
end
...
local mfceore = colour_end_of_round_effects
...
SMODS.Joker{
	...
	unlocked = true,
	in_pool = function()
		return togabalatro.mf_colours_held()
	end,
	rarity = 1,
		...
		if (context.end_of_round or context.forcetrigger) and not context.repetition and not context.individual and not context.game_over then
			...
			for i = 1, #G.consumeables.cards do
				if G.consumeables.cards[i].config.center.set == 'Colour' then
					local effects = { dollars = card.ability.extra.percolormoney, message_card = G.consumeables.cards[i], juice_card = context.retrigger_joker or context.blueprint_card or card }
					local stacked, stackamount = togabalatro.stackingcompat(G.consumeables.cards[i])
					if stacked then
						for c = 1, stackamount do
							table.insert(ceff, effects)
						end
					else
						table.insert(ceff, effects)
					end
			...
			if next(ceff) then return SMODS.merge_effects(ceff) end
		end
	end,
