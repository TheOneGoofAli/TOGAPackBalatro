...
togabalatro.jd_def["j_toga_jokersrb2kart"] = {
	...
	calc_function = function(card)
		...
		card.joker_display_values.active = type(poker_hands) == 'table' and poker_hands['Straight'] and next(poker_hands['Straight']) and true or false
	end,
	style_function = function(card, text, reminder_text, extra)
...
togabalatro.jd_def["j_toga_spacecadetpinball"] = {
	...
	calc_function = function(card)
		...
		card.joker_display_values.rescores = math.max(togabalatro.cashpointmulitple(card.ability.extra.cashpoint), 0)
		card.joker_display_values.odds = localize { type = 'variable', key = "jdis_odds", vars = { SMODS.get_probability_vars(card, 1, card.ability.extra.odds) } }
	end,
	style_function = function(card, text, reminder_text, extra)
...
togabalatro.jd_def["j_toga_whatisthis"] = {
	text = {
		{
			border_nodes = {
				{ text = "^" },
				{ ref_table = "card.joker_display_values", ref_value = "part" },
			},
	...
	calc_function = function(card)
		local cons = 0
		card.joker_display_values.part = card.ability.extra.part^JokerDisplay.calculate_joker_triggers(card)
...
togabalatro.jd_def["j_toga_victor"] = {
	reminder_text = {
		{ text = "(", scale = 0.35 },
		{ text = localize('Full House', "poker_hands"), scale = 0.35 },
		{ text = ")", scale = 0.35 },
	},
	calc_function = function(card)
		local _, poker_hands, _ = JokerDisplay.evaluate_hand()
		card.joker_display_values.fh = poker_hands["Full House"] and next(poker_hands["Full House"]) and true or false
	end,
	style_function = function(card, text, reminder_text, extra)
		if reminder_text and reminder_text.children[1] and reminder_text.children[2] and card.joker_display_values then
			reminder_text.children[2].config.colour = card.joker_display_values.fh and G.C.FILTER or G.C.UI.TEXT_INACTIVE
		end
		return false
	end
}
...
togabalatro.jd_def["j_toga_winamp"] = {
	text = {
		...
		{ text = " ", colour = G.C.UI.TEXT_INACTIVE },
		{ text = "(", colour = G.C.UI.TEXT_INACTIVE },
		{ ref_table = "card.joker_display_values", ref_value = "txt", colour = G.C.UI.TEXT_INACTIVE },
		{ text = ")", colour = G.C.UI.TEXT_INACTIVE },
	},
	calc_function = function(card)
		local llama = string.find(string.lower(love.system.getClipboardText() or ""), "mike")
		card.joker_display_values.xmult = llama and card.ability.extra.xmult or 1
		card.joker_display_values.active = llama and true or false
		card.joker_display_values.txt = llama and "!" or "?"
	end,
	style_function = function(card, text, reminder_text, extra)
		if text and text.children[1] and text.children[4] and card.joker_display_values then
			text.children[4].config.colour = card.joker_display_values.active and G.C.FILTER or G.C.UI.TEXT_INACTIVE
		end
		return false
	end
}

togabalatro.jd_def["j_toga_winrar"] = {
	text = {
		{
			border_nodes = {
				{ text = "X" },
	...
	calc_function = function(card)
		card.joker_display_values.xmult = 1+card.ability.extra.xm
	end,
}
...
togabalatro.jd_def["j_toga_mario_cdi"] = {
	text = {
		{ ref_table = "card.joker_display_values", ref_value = "mtxt", colour = G.C.MONEY },
	},
	reminder_text = {
		{ text = "(", scale = 0.3 },
		{ ref_table = "card.joker_display_values", ref_value = "txt", colour = { ref_table = G.ARGS.LOC_COLOURS, ref_value = "toga_suitgradient" }, scale = 0.3 },
		{ text = ")", scale = 0.3 },
	},
	calc_function = function(card)
		card.joker_display_values.mtxt = card.joker_display_values.mtxt or SMODS.signed_dollars(card.ability.extra.money)
		card.joker_display_values.txt = card.joker_display_values.txt or localize('toga_targetsuitjkr')
	end,
}
